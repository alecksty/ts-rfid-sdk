    ...
    public static void main(String[] args) {
        ...
        rfidSystem = new RfidSystem(null, strRfid, strPort
            ...
            if (obj instanceof TagItem) {
                ...
                synchronized (rfidSystem.getRfidManager().getList()) {
                    //  遍历没有输出的
                    for (TagItem t : rfidSystem.getRfidManager().getList()) {
                        if (!t.selected) {
                            t.selected = true;
                            System.err.println("EPC:" + t.getTextEpc() + ",PC:" + String.format("%04x", 0xffff & t.pc) + ",CRC:" + String.format("%04x", 0xffff & t.crc) + ",RSSI:" + t.rssi);
                        }
                    }
                }
            }
        ...
        while (portMan.getPort().isConnected()) {
            ...
            //  数量改变
            if (sz != rfidSystem.getRfidManager().getList().size()) {
                sz = rfidSystem.getRfidManager().getList().size();
