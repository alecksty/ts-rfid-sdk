...
public class ContentActivity extends AppCompatActivity implements View.OnClickListener, IDlgEvent, IRfidEvent {
    ...
    protected void onStart() {
        ...
        RfidManager man = theApp.rfidSystem.getRfidManager();
        //  清空指令
        man.cleanCmd();
        //  读写器
     ...
     * @return : 成功/失败
     */
    private boolean tagReadAll(final TagItem tag) {
        //  事件接口
        RfidApp theApp = (RfidApp) this.getApplication();
        //  读写器
        BaseReader reader = theApp.rfidSystem.getRfidManager().getReader();
        //  命令对象
        BaseCmd cmd = reader.getCmd();
        //  暂停
        theApp.rfidSystem.pause();
        //  清空命令队列
        theApp.rfidSystem.getRfidManager().cleanCmd();
        //  发送停止扫描指令
        theApp.portWrite(cmd.rfid_stop(), 200);

        //  设置选择
        theApp.portWrite(cmd.rfid_set_select(BaseCmd.RFID_BANK_EPC, 0, tag.epc), 300);

        //  轮询一次
        //theApp.portWrite(cmd.rfid_inventory(1), 300);

        //  读取等待
        //theApp.portWrite(cmd.rfid_get_select(), 400);
