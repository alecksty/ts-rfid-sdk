...
public class SetupActivity extends AppCompatActivity implements ISetupEven, View.OnClickListener, IDlgEvent, IDlgUser, IPortEvent, IConfigEvent {
    ...
    //  等待200ms，一定要加延时，不加，某些数据会错乱，因为单片机可能会处理不过来！
    private static final int CONFIG_DELAY = 200;
    ...
    private static final int XITEM_CONFIG_WIFI_READ = 3070;
    private static final int XITEM_CONFIG_WIFI_WRITE = 3080;
    //  4G配置
    private static final int XITEM_CONFIG_CON_TYPE = 3061;
    private static final int XITEM_CONFIG_APN_NAME = 3062;
    private static final int XITEM_CONFIG_DEV_NAME = 3063;
    private static final int XITEM_CONFIG_4G_READ = 3064;
    private static final int XITEM_CONFIG_4G_WRITE = 3065;
    //  缓存模式
    private static final int XITEM_CONFIG_BUFFER_START = 3090;
    private static final int XITEM_CONFIG_BUFFER_STOP = 3091;
    private static final int XITEM_CONFIG_BUFFER_CLEAN = 3092;
    private static final int XITEM_CONFIG_BUFFER_TEST = 3093;
    //  区域
    ...
    protected void onResume() {
        ...
        //  拦截端口消息
        theApp.rfidSystem.getPortManager().setPortEvent(this);
        //  加载菜单 - 刷新
        initRootMenu();
    }
    ...
    private void initUartMenu() {
        RfidApp theApp = (RfidApp) this.getApplication();
        PortUART portUart = (PortUART) theApp.rfidSystem.getPortManager().getPort(EPortType.PORT_TYPE_UART);
        //  获取列表
    ...
    private void initNetworkMenu() {
        RfidApp theApp = (RfidApp) this.getApplication();
        PortManager port = theApp.rfidSystem.getPortManager();
    ...
    private void initUsbMenu() {
        RfidApp theApp = (RfidApp) this.getApplication();
        PortUSB portUSB = (PortUSB) theApp.rfidSystem.getPortManager().getPort(EPortType.PORT_TYPE_USB);
    ...
    private void initRootMenu() {
        ...
        if (manager == null) {
            ...
            if (curItemId == 0) {
                ...
                list.add(new XSetupItem(XITEM_INSTRUCTION, getString(R.string.str_setup_instruction), EReaderType.toValue(theApp.rfidSystem.getRfidManager().getType()), arrayInstruct));
                ...
                list.add(new XSetupItem(XITEM_CONFIG, getString(R.string.str_setup_config), new XSetupItem[]{
                        ...
                        new XSetupItem(XITEM_CONFIG_WIFI_READ, getString(R.string.str_config_read)),
                        new XSetupItem(XITEM_CONFIG_WIFI_WRITE, getString(R.string.str_config_write)),

                        new XSetupItem(getString(R.string.str_setup_4g)),
                        new XSetupItem(XITEM_CONFIG_CON_TYPE, getString(R.string.str_setup_con_type), reader.set.set_con_type, "GPRS"),
                        new XSetupItem(XITEM_CONFIG_APN_NAME, getString(R.string.str_setup_apn_name), reader.set.set_apn_name, "DNMS"),
                        new XSetupItem(XITEM_CONFIG_DEV_NAME, getString(R.string.str_setup_dev_name), reader.set.set_dev_name, "DOOR-01"),
                        new XSetupItem(XITEM_CONFIG_4G_READ, getString(R.string.str_config_read)),
                        new XSetupItem(XITEM_CONFIG_4G_WRITE, getString(R.string.str_config_write)),

                        new XSetupItem(getString(R.string.str_setup_buffer)),
                        new XSetupItem(XITEM_CONFIG_BUFFER_START, getString(R.string.str_setup_buffer_start)),
                        new XSetupItem(XITEM_CONFIG_BUFFER_STOP, getString(R.string.str_setup_buffer_stop)),
                        new XSetupItem(XITEM_CONFIG_BUFFER_CLEAN, getString(R.string.str_setup_buffer_clean)),
                        new XSetupItem(XITEM_CONFIG_BUFFER_TEST, getString(R.string.str_setup_buffer_test)),
                        new XSetupItem(""),
                }));
    ...
    public void OnSetupEvent(int msg, XSetupItem item, int id, boolean on, int pos, String text, Object param) {
        ...
        switch (id) {
            ...
            case XITEM_USART_OPEN:
                if (!portUart.isConnected()) {
                    try {
                        ...
                        if (portUart.connect()) {
                            //  端口
                            theApp.rfidSystem.getPortManager().setType(EPortType.PORT_TYPE_UART);
                            theApp.rfidSystem.getRfidManager().setPort(theApp.rfidSystem.getPortManager().getPort());
            ...
            case XITEM_USB_OPEN:
                ...
                if (portUsb.isConnected()) {
                    theApp.rfidSystem.getPortManager().setType(EPortType.PORT_TYPE_USB);
                    theApp.rfidSystem.getRfidManager().setPort(theApp.rfidSystem.getPortManager().getPort());
            ...
            case XITEM_INSTRUCTION:
                ...
                //  需要从新初始化
                theApp.rfidSystem.setInit(false);
                //  切换
            ...
            case XITEM_CONFIG_CON_TYPE:
                LoggerActivity.log_d(TAG, "承载类型:" + item.textValue);
            ...
            case XITEM_CONFIG_APN_NAME:
                LoggerActivity.log_d(TAG, "APN:" + item.textValue);
            ...
            case XITEM_CONFIG_DEV_NAME:
                LoggerActivity.log_d(TAG, "DEV NAME:" + item.textValue);
            ...
            //===============
            case XITEM_INVENTRY:
                LoggerActivity.log_d(TAG, "单个天线轮询次数:" + pos);
                //  每次轮询次数
            ...
            //===============
            case XITEM_INTERVAL:
                LoggerActivity.log_d(TAG, "切换天线间隔:" + pos);
                set.sw_interval = pos;
                rfidMan.setupUpdate(true);
                manager.update();
                break;
    ...
    public void OnPortEvent(EPortType type, EPortEven even, int param, Object device, String message) {
        ...
        switch (even) {
            ...
            case PORT_EVENT_CONNECTED:
                ...
                new MsgMake(this, EMessageType.MSG_SUCCESS, getString(R.string.str_connected), MESSAGE_DELAY_CNT);
                break;
     ...
     * @param man ： 管理器
     */
    private synchronized void Config_Action_Wifi_Read(RfidManager man) {

        System.err.println("Config_Action_Wifi_Read()");
    ...
    private void Config_Action_Wifi_Write(RfidManager man) {
        ...
        if (DEBUG) {
            System.err.println("Config_Action_Wifi_Write()");
        }
     ...
     * @param manager : 管理器
     */
    private synchronized void Config_Action_4G_Read(RfidManager manager) {

        System.err.println("Config_Action_4G_Read()");
    ...
    private void Config_Action_4G_Write(RfidManager man) {
        ...
        if (DEBUG) {
            System.err.println("Config_Action_4G_Write()");
        }
        ...
        //  4G设置
        byte[] FixCon = makeFixData(set.set_con_type, 16);
        byte[] FixApn = makeFixData(set.set_apn_name, 16);
        byte[] FixDev = makeFixData(set.set_dev_name, 16);
    ...
    private void Config_Action_Command(RfidManager man, int cmd) {
        ...
        if (DEBUG) {
            System.err.println("Config_Action_Command()");
        }
    ...
    public void OnConfigEvent(RfidConfig config, int type, int offset, byte[] data) {
        ...
        if (DEBUG) {
            System.err.println("OnConfigEvent(type:" + type + ")");
        }
